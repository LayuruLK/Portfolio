...
const Contact = ({isDarkMode}:ContactProps) => {
  ...
  const onSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (data.success) {
      ...
      form.reset(); // TypeScript now knows 'form' is HTMLFormElement
    } else {
  ...
  return (
    <div id='contact' className='w-full px-[12%] py-10 scroll-mt-20 bg-no-repeat bg-center bg-cover font-outfit'
      ...
      <form onSubmit={onSubmit} action="" className='max-w-2xl mx-auto'>
        <div className='grid grid-cols-[var(--grid-template-columns-layout)] gap-6 mt-10 mb-8'>
        ...
        <textarea rows={6} placeholder='Enter your message' required name="message" id="" className='w-full p-4 outline-none border-[0.5px] border-gray-400 rounded-md bg-white mb-6'></textarea>
